...
🇨🇳 基于 WebDAV 的终端文件同步工具 | 🇺🇸 A WebDAV-based terminal file sync tool
...
- 📦 **分块传输** — 700KB/块的分块上传/下载，确保传输稳定性
- 🔒 **MD5 校验** — 完整性验证，确保文件无损传输
- ⚡ **冲突检测** — 智能识别本地与远程的文件冲突，支持手动解决
- ⏱️ **自动同步** — 可配置的自动同步倒计时
- 🌐 **国际化** — 支持中/英文界面一键切换（`L` 键）
...
<basePath>/
  ...
  data/
    file_<id>          ← 文件分块数据 (Base64 编码)
    file_<id>_1        ← 多块时的后续分块
```
...
- 📦 **Chunked Transfer** — 700KB per chunk for stable uploads/downloads
...
- ⏱️ **Auto Sync** — Configurable automatic sync countdown
...
| ⬆️ To Upload | Local has changes |
| ⬇️ To Download | Cloud has changes |
| 🆕 First Upload | Not yet uploaded to cloud |
| ❓ Missing | Local file absent, can restore from cloud |
| ⚠️ Conflict | Both sides modified, manual resolution needed |
...
<basePath>/
  ...
  data/
    file_<id>          ← File chunk data (Base64 encoded)
    file_<id>_1        ← Subsequent chunks for large files
```
